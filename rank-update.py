...
total_steps = len(RANK_THRESHOLDS) + 2
step = 0
...
for rank_name, threshold in RANK_THRESHOLDS:
    ...
    rank_results[rank_name] = players
    # Lower thresholds always have an equal-or-later boundary page.
    ...
    if SHOW_PROGRESS:
        print_progress(
            step,
            total_steps,
            f"{rank_name} cutoff ({threshold:,}) found: {players:,} players",
        )
    else:
    ...
    "ranks": [
        {
            ...
            "players_qualified": rank_results[rank_name],
        }
        for rank_name, threshold in RANK_THRESHOLDS
    ],
}
